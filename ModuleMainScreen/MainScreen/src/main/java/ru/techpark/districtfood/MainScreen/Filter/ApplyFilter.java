package ru.techpark.districtfood.MainScreen.Filter;

import android.content.Intent;
import android.os.Bundle;
import android.util.Log;

import com.google.android.gms.common.server.converter.StringToIntConverter;

import java.util.ArrayList;
import java.util.List;

import ru.techpark.districtfood.Constants;
import ru.techpark.districtfood.MainScreen.CardsPreview.Card;

public class ApplyFilter {

    private List<Card> cards;

    public ApplyFilter() { }

    public List<Card> apply(Bundle BundleKeyFilter){

        List<Card> newCards_MR = new ArrayList<>();
        List<Card> newCards_S = new ArrayList<>();
        List<Card> newCards_TFF = new ArrayList<>();
        List<Card> newCards_TS = new ArrayList<>();
        List<Card> newCards_TWI = new ArrayList<>();

        boolean tag_with_itself = BundleKeyFilter.getBoolean(Constants.TAG_WITH_ITSELF, false);
        boolean tag_fast_food = BundleKeyFilter.getBoolean(Constants.TAG_FAST_FOOD, false);
        boolean tag_sale = BundleKeyFilter.getBoolean(Constants.TAF_SALE, false);
        float number_star = BundleKeyFilter.getFloat(Constants.NUMBER_STAR, 0);
        String text_middle_receipt = BundleKeyFilter.getString(Constants.TEXT_MIDDLE_RECEIPT, "");

        if (!text_middle_receipt.equals("")) {
            for (Card card : cards) {
                if (card.getMiddle_receipt() <= Integer.parseInt(text_middle_receipt)) {
                    newCards_MR.add(card);
                }
            }
        }else newCards_MR = cards;

        if (number_star != 0) {
            for (Card card : cards) {
                if (card.getScore() >= number_star) {
                    for (Card card1 : newCards_MR) {
                        if (card == card1) {
                            newCards_S.add(card);
                            break;
                        }
                    }
                }
            }
        }else newCards_S = newCards_MR;

        if (tag_fast_food) {
            for (Card card : cards) {
                if (card.getTagFastFood()) {
                    for (Card card1 : newCards_S) {
                        if (card == card1) {
                            newCards_TFF.add(card);
                            break;
                        }
                    }
                }
            }
        }else newCards_TFF = newCards_S;

        if (tag_sale) {
            for (Card card : cards) {
                if (card.getTagSale()) {
                    for (Card card1 : newCards_TFF) {
                        if (card == card1) {
                            newCards_TS.add(card);
                            break;
                        }
                    }
                }
            }
        }else newCards_TS = newCards_TFF;

        if (tag_with_itself) {
            for (Card card : cards) {
                if (card.getTagWithItself()) {
                    for (Card card1 : newCards_TS) {
                        if (card == card1) {
                            newCards_TWI.add(card);
                            break;
                        }
                    }
                }
            }
        }else newCards_TWI = newCards_TS;

        return newCards_TWI;

    }

    public static ApplyFilter sInstance;

    public synchronized static ApplyFilter getInstance(){
        if (sInstance == null) {
            sInstance = new ApplyFilter();
        }
        return sInstance;
    }

    public void SetCards(List<Card> cards){
        this.cards = cards;
    }

}
